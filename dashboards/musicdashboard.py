...
def stats_top_countries_plot():
    ...
    top_countries_df = dbconnect.run_query_to_df("SELECT artistcountry AS 'Country', COUNT(*) AS 'Stats' FROM `tbl_artist` GROUP BY artistcountry ORDER BY Stats DESC LIMIT 10;", connection=cnx)
...
def stats_tracks_plot():
    ...
    longest_rel_df = dbconnect.run_query_to_df("SELECT DISTINCT `Artist`, `Release Name`, `Total Duration` FROM `durations_view` ORDER BY `Total Duration` DESC LIMIT 1;", connection=cnx)
    shortest_rel_df = dbconnect.run_query_to_df("SELECT DISTINCT `Artist`, `Release Name`, `Total Duration` FROM `durations_view` WHERE `Total Duration` IS NOT NULL ORDER BY `Total Duration` ASC LIMIT 1;",connection=cnx)
